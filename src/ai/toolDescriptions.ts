...
export const AGENT_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    "motion.set-clip-range": "重定时运镜片段(起止/时长),轨迹形状不变;跟随态 Program 片段一并移动",
    "motion.set-key": "落一枚镜头关键帧(同 id 覆盖);视口摆位/时间轴打点/AI 共用写入口",
    "motion.move-key": "移动关键帧的 progress,调整段间节奏(不改变轨迹形状)",
    ...
    "motion.author": "语义运镜编译:推近/拉远/摇/俯仰/横移/升降/环绕/定镜,产物为可再编辑关键帧",
    "motion.quick-author": "按被摄体+景别+运动一步生成运镜(机位定距与关键帧一次到位)",
    "program.set-clip": "把运镜片段排入 Program 时段(同一时段只允许一个机位输出)",
    "program.remove-clip": "从 Program 移除片段(运镜片段本身保留)",
    "motion.preview.enter": "进入片段预览,playhead 不在片段内时自动 seek 到片段起点",
    ...
    // 时间轴
    "timeline.add-key": "时间轴加走位关键帧(轨道不存在则按 targetId 创建)",
    "timeline.move-key": "移动时间轴关键帧的时刻",
    "timeline.remove-key": "删除时间轴关键帧;轨道清空后保留",
    "timeline.set-key-easing": "设时间轴关键帧缓动(linear/smooth)",
    "timeline.set-duration": "设时间轴总时长(秒),playhead 钳入新范围",
    "timeline.restore-tracks": "整体恢复时间轴轨道(撤销删除/文档导入的配套回放)",
    // 动作与播放
    ...
    "action.preview.seek": "局部动作预览定位到指定秒",
    "transport.play": "播放全局时间轴",
    "transport.pause": "暂停播放,playhead 停驻当前时刻",
    "transport.stop": "停止并回 0,清除时间轴采样、恢复实体权威变换",
    "transport.seek": "定位 playhead 到指定秒(双端钳入时长范围)",
    "transport.set-loop": "整段循环开关(编排期反复评估节奏用)",
    // 人偶
    ...
    "capture.video": "录制预演 WebM(缺省时长=时间轴时长);异步产物按 requestId 对账",
    "capture.video-cancel": "提前终止录制并丢弃产物",
    // 演示与文档
