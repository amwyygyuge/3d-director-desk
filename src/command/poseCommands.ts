...
interface SetWeightPayload {
    readonly objectId: string;
    readonly weight: number;
}
interface AddPoseKeyPayload {
    readonly trackId: string;
    readonly targetId: string;
    readonly keyframe: {
        readonly id: string;
        readonly time: number;
        readonly value: PoseSnapshotInit;
        readonly easing: TimelineEasing;
    };
}
interface MovePoseKeyPayload {
    readonly trackId: string;
    readonly keyframeId: string;
    readonly time: number;
}
interface RemovePoseKeyPayload {
    readonly trackId: string;
    readonly keyframeId: string;
}
interface SetPoseKeyEasingPayload {
    readonly trackId: string;
    readonly keyframeId: string;
    readonly easing: TimelineEasing;
}
...
function objectIssue(ctx: DirectorContext, objectId: unknown): CommandIssue | null {
    ...
    if (!entity || entity.kind !== "model")
        return issue("pose-model-not-found", "objectId", "姿态只能应用到存在的模型对象");
    return null;
}

function editingIssue(ctx: DirectorContext): CommandIssue | null {
    return ctx.clock.isPlaying
        ? issue("pose-playback-active", "", "播放期间不能编辑姿态，请先暂停", [
...
function timelineKeyIssue(ctx: DirectorContext, trackId: unknown, keyframeId: unknown): CommandIssue | null {
    if (typeof trackId !== "string" || trackId.length === 0)
        return issue("pose-invalid-track", "trackId", "姿态轨道 id 格式无效");
    if (typeof keyframeId !== "string" || keyframeId.length === 0)
        return issue("pose-invalid-key", "keyframeId", "姿态关键帧 id 格式无效");
    const track = ctx.timeline.document.track(trackId);
    if (!track || track.kind !== TIMELINE_TRACK_KIND.POSE)
        return issue("pose-track-not-found", "trackId", "姿态轨道不存在");
...
export class SetPoseWeightCommand extends DirectorCommand<SetWeightPayload> {
    ...
    constructor(readonly payload: SetWeightPayload) {
        super();
    }
    validate(ctx: DirectorContext): string[] {
        return messages(this.validateIssues(ctx));
    ...
    execute(ctx: DirectorContext): void {
        ctx.scene.setObjectPoseWeight(this.payload.objectId, this.payload.weight);
        ctx.playback.sampleCurrent();
    }
    override invert(ctx: DirectorContext): readonly SerializedCommand[] | null {
        const entity = ctx.scene.manager.getEntity(this.payload.objectId);
        return entity
            ? [{ type: SetPoseWeightCommand.TYPE, payload: { objectId: entity.id, weight: entity.poseWeight } }]
            : null;
    }
}
...
export class AddPoseKeyCommand extends DirectorCommand<AddPoseKeyPayload> {
    ...
    constructor(readonly payload: AddPoseKeyPayload) {
        super();
    }
    validate(ctx: DirectorContext): string[] {
        return messages(this.validateIssues(ctx));
    ...
    override validateIssues(ctx: DirectorContext): readonly CommandIssue[] {
        const editing = editingIssue(ctx);
        if (!isRecord(this.payload) || !isRecord(this.payload.keyframe))
            return [issue("pose-invalid-payload", "", "姿态关键帧参数无效")];
        if (editing) return [editing];
        const target = objectIssue(ctx, this.payload.targetId);
        if (target) return [target];
        if (typeof this.payload.trackId !== "string" || this.payload.trackId.length === 0)
            return [issue("pose-invalid-track", "trackId", "姿态轨道 id 格式无效")];
        ...
        if (
            ...
            key.time < 0 ||
            key.time > ctx.timeline.document.duration
        )
            return [issue("pose-invalid-key", "keyframe", "姿态关键帧格式或时间无效")];
        if (key.easing !== TIMELINE_EASING.LINEAR && key.easing !== TIMELINE_EASING.SMOOTH)
            return [issue("pose-invalid-key", "keyframe.easing", "姿态关键帧缓动无效")];
        ...
        if (track && (track.kind !== TIMELINE_TRACK_KIND.POSE || track.targetId !== this.payload.targetId))
            return [issue("pose-track-conflict", "trackId", "姿态轨道已属于其他目标或类型")];
        if (track?.keyframe(key.id) || track?.keyframes.some((current) => current.time === key.time))
            return [issue("pose-duplicate-key", "keyframe", "姿态轨道中的关键帧 id 和时间必须唯一")];
        return ctx.timeline.document.trackForTarget(this.payload.targetId, TIMELINE_TRACK_KIND.POSE) && !track
            ? [issue("pose-track-conflict", "targetId", "对象已有姿态轨道")]
            : [];
    }
    execute(ctx: DirectorContext): void {
        ctx.timeline.addPoseKey(this.payload.trackId, this.payload.targetId, new PoseKeyframe(this.payload.keyframe));
        ctx.playback.sampleCurrent();
    }
    override invert(): readonly SerializedCommand[] {
        return [
            {
                type: RemovePoseKeyCommand.TYPE,
...
export class MovePoseKeyCommand extends DirectorCommand<MovePoseKeyPayload> {
    ...
    constructor(readonly payload: MovePoseKeyPayload) {
        super();
    }
    validate(ctx: DirectorContext): string[] {
        return messages(this.validateIssues(ctx));
    ...
    override validateIssues(ctx: DirectorContext): readonly CommandIssue[] {
        ...
        if (key) return [key];
        if (
            !Number.isFinite(this.payload.time) ||
            this.payload.time < 0 ||
            this.payload.time > ctx.timeline.document.duration
        )
            return [issue("pose-invalid-time", "time", "姿态关键帧时间超出时间轴")];
        const track = ctx.timeline.document.track(this.payload.trackId)!;
        return track.keyframes.some(
            (current) => current.id !== this.payload.keyframeId && current.time === this.payload.time,
        )
            ? [issue("pose-duplicate-time", "time", "姿态关键帧时间必须唯一")]
            : [];
    }
    execute(ctx: DirectorContext): void {
        const key = ctx.timeline.document.track(this.payload.trackId)?.keyframe(this.payload.keyframeId) as
            PoseKeyframe | undefined;
        if (key)
            ctx.timeline.moveKey(this.payload.trackId, new PoseKeyframe({ ...key.toJSON(), time: this.payload.time }));
        ctx.playback.sampleCurrent();
    }
    override invert(ctx: DirectorContext): readonly SerializedCommand[] | null {
        const key = ctx.timeline.document.track(this.payload.trackId)?.keyframe(this.payload.keyframeId);
        return key ? [{ type: MovePoseKeyCommand.TYPE, payload: { ...this.payload, time: key.time } }] : null;
    }
}
...
export class RemovePoseKeyCommand extends DirectorCommand<RemovePoseKeyPayload> {
    ...
    constructor(readonly payload: RemovePoseKeyPayload) {
        super();
    }
    validate(ctx: DirectorContext): string[] {
        return messages(this.validateIssues(ctx));
    ...
    execute(ctx: DirectorContext): void {
        ...
        if (targetId) ctx.playback.sampleCurrent();
    }
    override invert(ctx: DirectorContext): readonly SerializedCommand[] | null {
        const track = ctx.timeline.document.track(this.payload.trackId);
        const key = track?.keyframe(this.payload.keyframeId);
        return track && key
            ? [
                  {
                      type: AddPoseKeyCommand.TYPE,
                      payload: {
                          trackId: track.id,
                          targetId: track.targetId,
                          keyframe: (key as PoseKeyframe).toJSON(),
                      },
                  },
              ]
            : null;
...
export class SetPoseKeyEasingCommand extends DirectorCommand<SetPoseKeyEasingPayload> {
    ...
    constructor(readonly payload: SetPoseKeyEasingPayload) {
        super();
    }
    validate(ctx: DirectorContext): string[] {
        return messages(this.validateIssues(ctx));
    ...
    override validateIssues(ctx: DirectorContext): readonly CommandIssue[] {
        ...
        if (editing) return [editing];
        if (key) return [key];
        return this.payload.easing === TIMELINE_EASING.LINEAR || this.payload.easing === TIMELINE_EASING.SMOOTH
            ? []
            : [issue("pose-invalid-easing", "easing", "姿态关键帧缓动无效")];
    }
    execute(ctx: DirectorContext): void {
        const key = ctx.timeline.document.track(this.payload.trackId)?.keyframe(this.payload.keyframeId) as
            PoseKeyframe | undefined;
        if (key)
            ctx.timeline.moveKey(
                ...
                new PoseKeyframe({ ...key.toJSON(), easing: this.payload.easing }),
            );
        ctx.playback.sampleCurrent();
    }
    override invert(ctx: DirectorContext): readonly SerializedCommand[] | null {
        const key = ctx.timeline.document.track(this.payload.trackId)?.keyframe(this.payload.keyframeId);
        return key ? [{ type: SetPoseKeyEasingCommand.TYPE, payload: { ...this.payload, easing: key.easing } }] : null;
    }
}
...
export class GetPoseQuery implements DirectorQuery<ObjectPayload> {
    ...
    execute(ctx: DirectorContext): unknown {
        const entity = ctx.scene.manager.getEntity(this.payload.objectId)!;
        return { objectId: entity.id, pose: entity.pose?.toJSON() ?? null, poseWeight: entity.poseWeight };
    }
}
...
export function registerPoseCommands(dispatcher: CommandDispatcher): void {
    ...
    dispatcher.register(
        SetPoseWeightCommand.TYPE,
        (payload: SetWeightPayload) => new SetPoseWeightCommand(payload),
        capability(SetPoseWeightCommand.TYPE, "command", [EDIT_PERMISSION]),
    );
    dispatcher.register(
        AddPoseKeyCommand.TYPE,
        (payload: AddPoseKeyPayload) => new AddPoseKeyCommand(payload),
        capability(AddPoseKeyCommand.TYPE, "command", [EDIT_PERMISSION]),
    );
    dispatcher.register(
        MovePoseKeyCommand.TYPE,
        (payload: MovePoseKeyPayload) => new MovePoseKeyCommand(payload),
        capability(MovePoseKeyCommand.TYPE, "command", [EDIT_PERMISSION]),
    );
    dispatcher.register(
        RemovePoseKeyCommand.TYPE,
        (payload: RemovePoseKeyPayload) => new RemovePoseKeyCommand(payload),
        capability(RemovePoseKeyCommand.TYPE, "command", [EDIT_PERMISSION]),
    );
    dispatcher.register(
        SetPoseKeyEasingCommand.TYPE,
        (payload: SetPoseKeyEasingPayload) => new SetPoseKeyEasingCommand(payload),
        capability(SetPoseKeyEasingCommand.TYPE, "command", [EDIT_PERMISSION]),
    );
