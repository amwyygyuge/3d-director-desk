...
export async function provisionAction(
    ...
    const handle = await ctx.models.acquire(init.url, format);
    try {
        const clip =
...
export async function mountWhenReady(ctx: DirectorContext, objectId: string, actionId: string): Promise<boolean> {
