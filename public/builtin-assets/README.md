...
设计取向:人物只保留**一个通用人形模型**；布景提供方块、平台、墙、立柱和四棱锥等低面数基础件；动作是独立的 Mixamo 兼容 FBX 资产,挂载前以目标骨架做世界空间重定向,并只保留旋转向量(源资产根位移单位不进入场景 transform)。模型面数是交互预算的一部分：人形资源不得以视觉细节换取缩放、旋转和拖拽帧率回退。
