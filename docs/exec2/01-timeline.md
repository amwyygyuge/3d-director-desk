...
- `TimelineStore`(MobX):持有 `TimelineDoc` 纯数据 + `revision` 锚定(SceneStore 先例);一切增删改走命令。
- `TimelineSampler`(领域服务):`transport.subscribe` 驱动;每帧对每轨道二分查找相邻关键帧 → 插值 → **直接写 SceneManager.runtimes**;零分配(模块级临时对象);不写 MobX、不产生命令。
...
3. `src/timeline/TimelineSampler.ts`:订阅 transport;回放期写运行时;`stop()` 时触发一次实体数据重渲染(scene.revision++)恢复编辑态画面。
