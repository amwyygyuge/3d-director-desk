...
做：机位、运镜与时间轴的分离编排；单一 Program 输出；导演/机位/成片三种视口模式；Bézier 运镜路径；世界点或场景对象注视绑定；静态姿势精修与灯光氛围。

不做：模型动作播放、对象变换关键帧、目标关键帧、速度曲线编辑器、镜头转场、多机位宫格、并行 Program/Preview 监看、AI 生成分镜、导出/协作、动作重定向。后续能力记录在 [`exec2/04-camera-future.md`](./exec2/04-camera-future.md)。
