...
`motion.set-key` 的 payload **恒为片段自身空间**——由 `clip.follow` 是否为空唯一决定,payload 里不带空间标志位。换算时刻取**当前播放头**,与视口渲染同参考系(见 §9.6)。
...
世界扫掠路径只在开关开启时构建,并在片段、主体轨与跟拍参数的不可变引用变化时整段重建,不随播放头更新。

**相对路径、关键帧球/手柄与绑带一律锚定当前播放头。** 跟拍生效的唯一视觉证据就是「拖时间轴时轨迹跟着人走」;曾试过把相对路径钉在片段起点以求形状稳定,结果作者拖播放头时人走了、运镜留在原地,看上去像跟拍失效——这条路已验证是错的,不要再走回去。

由此派生一条硬约束:**关键帧拖拽的世界→局部反解必须使用与渲染相同的时刻(当前播放头)**,否则松手瞬间球会跳。至于「边播边拖球会漂」的担忧不成立:拖球时播放头不动,播放期本就不该编辑。

播放期只原地改写既有 BufferAttribute(相对路径顶点、绑带两个顶点),不重建几何、不分配;播放头落在片段区间外时隐藏绑带。
