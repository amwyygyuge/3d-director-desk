...
世界扫掠路径只在开关开启时构建,并在片段、主体轨与跟拍参数的不可变引用变化时整段重建,但不随播放头更新。相对路径与关键帧球/手柄固定锚定在**片段起点时刻的主体位姿**,因为相对轨迹是片段的属性而不是此刻的属性;若以随播放头变化的参考系绘制静态形状,会把不同时刻的关键帧硬塞进同一瞬时参考系,既无物理意义又让作者拖不住球。它们只在片段、跟拍参数、关键帧或主体走位轨引用变化时重算,因此选中不跳、seek 不跳、拖动播放头时形状稳定。跟拍绑带则固定锚定在**当前播放头**,表达此刻相机与主体的关系;播放期只原地改写绑带的两个顶点,播放头落在片段区间外时隐藏绑带。关键帧拖拽的世界→局部反解也必须使用片段起点参考系,与渲染保持同一参考系,否则松手即跳。
