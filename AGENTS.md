...
15. **地基优先纪律**:新功能动工前先核查承载它的领域层是否齐备(领域概念/值对象/服务/命令/注册表项)。地基有缺口时**必须先补地基再搭功能**——禁止在 UI 层用多次 dispatch 串联本应是聚合命令的操作(撤销碎裂、AI 不可见、半途失败留孤儿数据),禁止为绕开地基缺口在组件里开旁路。聚合写操作落地为单个命令类,`invert` 一次性回滚全部副产物;地基变更与功能同批交付。
